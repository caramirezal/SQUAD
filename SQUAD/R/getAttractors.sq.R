...
getAttractors.sq <- function(net,type=c("synchronous","asynchronous"),
                             method=c("exhaustive","sat.exhaustive","heuristic",
                                      "sat.restricted","random","chosen"),
                             ...) {
        if ( ! ( class(net) %in% c("BooleanNetwork","squad") ) ) {
                stop("net most be a BooleanNetwork of squad object")
        }
        if ( class(net) == "squad" ) {
                net <- list("genes" = net$genes,
                            "interactions" = net$interactions,
                            "fixed" = net$fixed)
                class(net) <- "BooleanNetwork"
        }
        if ( type == "asynchronous" && method == "heuristic") {
                attractors <- heuristicSSS(net)
        } else {
...
heuristicSSS <- function(net,returnPlot=FALSE) {
        ## checks arguments
        ...
        if ( length(res) <= 0 ) {
                warning("OK! But no fixed points found!")
                return(list())
        }
