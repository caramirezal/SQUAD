        ...
        network <- function(times,state,parameters) {
                with(as.list(c(state,parameters)),{
                        ...
                        for (i in 1:length(net$genes)) {
                                ...
                                #hillInputNorm <- hillFun(state[regulators]) / hillFun(1)
                                newState[i] <- boolCubeFun(net$interactions[[i]]$func,hillFunNorm(state[regulators])  ) - gamma*state[i]
                        }
                        names(newState) <- net$genes
                        return(list(newState))
                })
        }
        class(network) <- "normHillCube"
        network
}
